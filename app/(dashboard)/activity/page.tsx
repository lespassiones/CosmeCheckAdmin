...
/** Score → soft chip tone, matching the cosme-check INCI score buckets. */
...
export default async function ActivityPage() {
  const [kpis, trend, recent, distributions, verdicts] = await Promise.all([
    fetchActivityKpis(),
    fetchActivityTrend(30),
    fetchRecentAnalyses(30),
    fetchActivityDistributions(),
    fetchCoherenceVerdictsToday(),
  ]);
  ...
  return (
    <>
      <PageHeader
        title="Activité"
        subtitle={`${formatInt(kpis.analysesToday)} analyse${kpis.analysesToday > 1 ? "s" : ""} aujourd'hui · vue temps réel, tous utilisateurs.`}
      />
      ...
      <SectionHeader
        ...
        subtitle={`Les ${recent.length} dernières analyses, tous utilisateurs confondus`}
      />
      {recent.length === 0 ? (
        <div className="mb-8">
          <EmptyState
      ...
      {/* ─── Coherence verdicts (today) ────────────────────────────────── */}
      <SectionHeader
        title="Verdicts cohérence — aujourd'hui"
        subtitle={`${formatInt(verdicts.total)} promesse${verdicts.total > 1 ? "s" : ""} analysée${verdicts.total > 1 ? "s" : ""}`}
      />
