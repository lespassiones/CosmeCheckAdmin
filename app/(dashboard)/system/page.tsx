...
// ──────────────────────────────────────────────────────────────────────────
// Local helpers (kept inline — `lib/utils.ts` is intentionally small)
// ──────────────────────────────────────────────────────────────────────────
...
/** Translate a 5-field cron expression to a short French label. */
...
/** Read the installed Next.js version from package.json (best-effort). */
...
const LARGE_TABLE_THRESHOLD_BYTES = 100 * 1024 * 1024; // 100 MB

export default async function SystemPage() {
  const [cron, tableStats, migrations, nextVersion] = await Promise.all([
    fetchCronJobs(),
    fetchTableStats(),
    fetchAppliedMigrations(),
    readNextVersion(),
  ...
  return (
    <>
      ...
      <div className="mb-8">
        {cron.needsMigration ? (
          ...
          <EmptyState
            ...
            description="Aucun job pg_cron correspondant n'a été trouvé. Vérifiez que les jobs sont bien préfixés `cosme_check_`."
          />
        ) : (
      ...
      {/* ─── D. Migrations ────────────────────────────────────────────── */}
      <SectionHeader
        title="Migrations appliquées"
        subtitle={`${formatInt(migrations.length)} migration${migrations.length > 1 ? "s" : ""} dans supabase_migrations.schema_migrations.`}
      />
      <div className="mb-8">
        {migrations.length === 0 ? (
          <EmptyState
      ...
      <div className="mb-8">
        {tableStats.needsMigration ? (
          <EmptyState
            icon={AlertTriangle}
            title="Migration 0009 requise"
            description="Le RPC public.cosme_check_admin_table_stats() n'existe pas encore. Appliquez `supabase/migrations/0009_admin_system_helpers.sql` pour activer cette section."
          />
        ) : tableStats.rows.length === 0 ? (
          <EmptyState
            icon={Database}
            title="Aucune table trouvée"
            description="Aucune table utilisateur dans `cosme_check` ou `public`. Vérifiez `pg_stat_user_tables`."
          />
        ) : (
          <article className="neo-card overflow-hidden">
            <div className="overflow-x-auto">
