...
export default async function CatalogProductsPage({
  ...
  const [kpis, list] = await Promise.all([
    fetchProductsKpis(),
    listProducts({ search, page, size, missingInci }),
  ]);

  // Helper to build paginated URLs while preserving filters.
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        subtitle={`${formatInt(kpis.total)} produits indexés${search ? ` · recherche "${search}"` : ""}${missingInci ? " · sans INCI" : ""}.`}
        actions={
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        <SectionHeader title="Liste" subtitle={`Page ${list.page} · ${formatInt(list.total)} résultat${list.total > 1 ? "s" : ""}`} />
        <Link
