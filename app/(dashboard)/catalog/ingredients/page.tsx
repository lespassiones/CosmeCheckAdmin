...
export default async function CatalogIngredientsPage({
  ...
  const [kpis, rows] = await Promise.all([
    fetchIngredientsKpis(),
    listIngredients({ search, view, limit: 200 }),
  ]);
  ...
  return (
    <>
      ...
      <PageHeader
        title="Ingrédients"
        subtitle={`${formatInt(kpis.total)} ingrédients INCI · ${formatInt(kpis.withExplanation)} avec explication AI.`}
        actions={
          <div className="flex items-center gap-2">
            ...
            <BatchExplainButton missing={kpis.withoutExplanation} />
          </div>
        }
      />
