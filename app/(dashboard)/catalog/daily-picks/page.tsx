...
export default async function DailyPicksPage({
  ...
  const [kpis, picks] = await Promise.all([
    fetchPicksKpis(),
    listPicks({ view, limit: 300 }),
  ]);
  ...
  return (
    <>
      ...
      <PageHeader
        title="Daily Picks"
        subtitle={`${formatInt(kpis.total)} cartes rotatives · ${formatInt(kpis.byKind.quiz)} quiz · ${formatInt(kpis.byKind.myth)} mythes.`}
      />
      ...
      {picks.length === 0 ? (
        ...
        <article className="neo-card overflow-hidden">
          <ul className="divide-y divide-black/[0.04]">
            {/* Header row — visual parity with a table header */}
